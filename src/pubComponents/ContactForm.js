  ...
  const FormField = ({ type, name, placeholder, className }) => {
    return (
      <input
        ...
        className={`${className} p-3 py-4 text-lg roboto-regular border-2 rounded-lg focus:outline-none border-none m-3 shadow-lg placeholder:font-handwriting placeholder:text-2xl`}
      />
    );
  };
    ...
    <>
      <div className="flex items-center justify-around w-full h-auto p-4 shadow-xl bg-primary-600 rounded-xl">
        <form className="flex flex-col items-center justify-between w-11/12 ">
          ...
          <textarea
            className="p-3 mx-4 placeholder:font-handwriting placeholder:text-2xl w-full text-lg border-primary-300 roboto-regular  rounded-lg h-[200px] shadow-lg focus:outline-none"
