...
export default function NumberedTimeline() {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-3/4 h-[500px] p-1  bg-white rounded-xl shadow-xl relative ">
      {data.map((event, index) => {
        return (
          <div
            ...
            className="flex items-center w-3/4 shrink-0 h-1/5"
          >
            ...
            <div className="flex justify-center w-1/3 shrink-0 ">
              <span className="text-2xl sm:whitespace-nowrap handwriting">
                {event.title}
              </span>
            </div>
          </div>
      ...
      <span
        style={lineStyle}
        className="h-5/6 bg-primary-600 w-[3px] absolute left-[50%] top-[50%] z-0"
      ></span>
    </div>
  );
}
